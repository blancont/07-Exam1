...
def problem3(point, length, delta, window):
    ...
    if round(length / delta) > length / delta:
        for k in range(round(length / delta)):
            line = rg.Line(rg.Point(point.x, point.y + (delta * k)),
                           rg.Point(point.x + (length + 20 * k), point.y + (delta * k)))
            line.thickness = 3
            if (k + 1) % 3 == 1:
                line.color = 'magenta'
            elif (k + 1) % 3 == 2:
        ...
        for k in range(round(length / delta) + 1):
            line = rg.Line(rg.Point(point.x, point.y + (delta * k)),
                           rg.Point(point.x + (length + 20 * k), point.y + (delta * k)))
            line.thickness = 3
            if (k + 1) % 3 == 1:
